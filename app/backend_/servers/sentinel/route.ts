...
async function fetchEmbedUrl(url: string): Promise<{
  embed_url: string;
  tracks: object[];
} | null> {
  const res = await fetch(url, {
  ...
  const embed_url = res.url;
  const html = await res.text();
  const tracksMatch = html.match(/var TRACKS\s*=\s*(\[.*?\]);/);
  const tracks = tracksMatch ? JSON.parse(tracksMatch[1]) : [];

  return { embed_url, tracks };
}
...
export async function GET(req: NextRequest) {
  try {
    ...
    const source = await fetchEmbedUrl(pageUrl);
